...
from vinbigdata.scaled_yolov4.models.experimental import attempt_load
...
def evaluate(
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            path, imgsz, batch_size, model.stride.max(), None, hyp=None, augment=False, cache=False, pad=0.5,
            rect=True)[0]

    seen = 0
    names = model.names if hasattr(model, 'names') else model.module.names
    s = ('%20s' + '%12s' * 6) % ('Class', 'Images', 'Targets', 'P', 'R', 'mAP@.4', 'mAP@.4:.95')
    ...
    if len(stats) and stats[0].any():
        ...
        p, r, ap50, ap = p[:, 0], r[:, 0], ap[:, 0], ap.mean(1)  # [P, R, AP@0.4, AP@0.4:0.95]
        mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
        nt = np.bincount(stats[3].astype(np.int64), minlength=data['nc'])  # number of targets per class
    else:
        nt = torch.zeros(1)
    ...
    if verbose and data['nc'] > 1 and len(stats):
